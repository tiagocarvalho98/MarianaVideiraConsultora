  ...
  return (
    <div className={compact ? "grid gap-2" : "grid gap-3 md:grid-cols-3"}>
      <form action={updateOpportunityStageAction}>
        ...
        <label className="text-xs font-bold uppercase text-stone-500" htmlFor={`stage-${opportunity.id}`}>
          Estado
        </label>
        <select
          ...
          className="mt-1 min-h-11 w-full rounded-xl border border-border bg-white px-3 text-sm font-semibold text-stone-800"
      ...
      <form action={setOpportunityTemperatureAction}>
        <input type="hidden" name="opportunityId" value={opportunity.id} />
        <label className="text-xs font-bold uppercase text-stone-500" htmlFor={`temperature-${opportunity.id}`}>
          Temperatura
        </label>
        <select
      ...
      <form action={assignOpportunityAction}>
        <input type="hidden" name="opportunityId" value={opportunity.id} />
        <label className="text-xs font-bold uppercase text-stone-500" htmlFor={`assigned-${opportunity.id}`}>
          Responsavel
        </label>
        <select
      ...
      {compact ? (
        <div className="flex gap-2">
          <form action={registerCallAction} className="flex-1">
            <input type="hidden" name="opportunityId" value={opportunity.id} />
            <button className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-border bg-white px-3 text-sm font-bold text-stone-700 transition hover:border-primary hover:text-primary">
